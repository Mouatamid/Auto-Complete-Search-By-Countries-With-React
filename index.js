...
class SearchBar extends React.Component {
  ...
  handleChange(e) {
		...
		for (let j = 0 ; j < countryList.length ; j++){
			if(countryList[j].name.toLowerCase().startsWith(e.target.value) &&
			e.target.value != "") i++;
		}
    this.props.handleInputChange(e.target.value, i);
  }
  render() {
    return (
...
class Country extends React.Component {
  ...
  listClicked(e){
    this.props.passCountry(e.target.textContent);
  }
  render() {
    return <li className="country" onClick={this.listClicked}>{this.props.value}</li>;
  }
}
class ListOfCountries extends React.Component {
  constructor(props){
    super(props);
    this.countryPass = this.countryPass.bind(this);
  }
  countryPass(e){
    this.props.passCountry(e);
  }
  render() {
    let inputValue = this.props.inputValue.toLowerCase();
    let matchedCountries = countryList.map((country) => {
      if (
        country.name.toLowerCase().startsWith(inputValue) &&
        inputValue != ""
      ) {
        return <Country value={country.name} key={country.code} passCountry={this.countryPass} />;
      }
    });
    let numberOfCountries = matchedCountries.length;
    return (
			<div>
				<ul className="listOfCountries">{matchedCountries}</ul>
				<Suggestions count={this.props.count} />
			</div>
		)
  }
}
class Suggestions extends React.Component {
  render() {
    return <div className="suggestions">Suggestions : {this.props.count}</div>;
  }
}
  ...
  render() {
    return (
      <div className="app">
        ...
        <SearchBar
          value={
            this.state.clickedValue == "" ? undefined : this.state.clickedValue
          }
          handleInputChange={this.changeInput}
        />
        <ListOfCountries inputValue={this.state.inputvalue} count={this.state.count} passCountry={this.clickedCountry}  />
